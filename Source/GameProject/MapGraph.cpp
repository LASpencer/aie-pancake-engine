...
std::stack<glm::vec2> MapGraph::aStarSearch(MapNode * startNode, MapNode * endNode, float(*heuristic)(MapNode *, MapNode*))
{
	MapNode* currentNode;
	auto gComp = [](MapNode* lhs, MapNode* rhs) {return lhs->fScore > rhs->fScore; };
	std::priority_queue<MapNode*, std::vector<MapNode*>, decltype(gComp)> openNodes(gComp);
	std::set<MapNode*> closedNodes;
	std::stack<glm::vec2> path;
	...
	while (!openNodes.empty()) {
		...
		for (MapEdge edge : currentNode->connections) {
			...
			if (closedNodes.count(targetNode) == 0) {
				...
				if (cost < targetNode->gScore) {
					targetNode->gScore = cost;
					targetNode->fScore = targetNode->gScore + heuristic(targetNode, endNode);
					targetNode->parent = currentNode;
				}
				if (isNotInQueue) {
					openNodes.push(targetNode);
...
{
	MapNode* newNode = new MapNode({position, 0, 0, nullptr, {}});
